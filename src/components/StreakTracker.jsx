...
const StreakTracker = ({ moodTracker }) => {
  ...
  return (
    <div className="bg-green-900 rounded-2xl shadow-lg p-6 text-white">
      ...
      <p className="text-sm"><span className="font-bold">Current Streak:</span> {currentStreak} days</p>
      <p className="text-sm"><span className="font-bold">Best Streak:</span> {bestStreak} days</p>
      {currentStreak >= 7 && (
        <p className="mt-2 text-yellow-300 font-semibold">🏆 Amazing! 7+ day streak!</p>
      )}
    </div>
  );
};
